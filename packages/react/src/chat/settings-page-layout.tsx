...
export function SettingsPageLayout({
  ...
  return (
    <div
      className={`@container/page-layout relative flex size-full min-h-0 flex-col ${className ?? ''}`}
    >
      {header}
      <div className="isolate flex min-h-0 flex-1">
        {sidebar && (
          <aside
            ...
            style={{ padding: 0, zIndex: 60, width: 250 }}
          >
            {sidebar}
          </aside>
        )}
        <div className="flex min-h-0 min-w-0 flex-1 flex-col">
          <main className="relative mt-0 flex-1 grow overflow-y-auto">{children}</main>
        </div>
      </div>
    </div>
  )
...
export function SettingsNavGroup({ title, children }: { title: string; children: ReactNode }) {
  return (
    <div className="flex w-full min-w-0 flex-col items-stretch gap-0.5">
      <p className="pb-1 px-1 text-label-12 font-medium text-v0-gray-600 uppercase">{title}</p>
      <ul className="flex w-full min-w-0 flex-col gap-1.5 gap-0.5!">{children}</ul>
    </div>
  )
}
  ...
  return (
    <header className="flex shrink-0 flex-col absolute inset-x-0 top-0 z-10 w-full border-t transition-colors sm:border-b sm:border-t-0 pointer-events-none border-transparent bg-transparent">
      <div className="relative shrink-0 items-center gap-3 pl-4 pr-3 sm:pl-3 sm:pr-2 sm:mx-0 hidden sm:flex h-[50px] hairline-b bg-v0-background-200">
        ...
        {logo && (
          <div className="group/sidebar-logo-toggle relative mr-1 hidden sm:flex">{logo}</div>
        )}
            ...
            <div className="pointer-events-auto min-w-0 justify-self-start pl-2">
              <nav aria-label="breadcrumb" data-slot="breadcrumb">
                <ol
                  ...
                  className="text-v0-gray-900 flex items-center text-sm break-words flex-nowrap gap-0"
                >
                  {breadcrumb}
                </ol>
              </nav>
            </div>
            <div className="pointer-events-auto min-w-0 justify-self-end pr-2">{actions}</div>
          </div>
        </div>
      </div>
    </header>
