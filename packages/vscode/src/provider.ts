...
export class V0ChatModelProvider implements LanguageModelChatProvider {
    ...
    try {
      ...
      if (response && response.choices && response.choices.length > 0) {
        ...
        if (message) {
          ...
          progress.report(new LanguageModelTextPart('Error: No response message from v0 API'))
        }
      } else {
        progress.report(new LanguageModelTextPart('Error: No response from v0 API'))
      }
    } catch (error) {
  ...
  private convertMessages(messages: Array<LanguageModelChatMessage>): V0ChatMessage[] {
