...
class V0ChatModelProvider {
  ...
  async provideLanguageModelChatResponse(model, messages, options, progress, token) {
    ...
    if (!apiKey) {
      progress.report(
        new vscode_1.LanguageModelTextPart(
    ...
    try {
      ...
      if (response && response.choices && response.choices.length > 0) {
        ...
        if (message) {
          ...
          progress.report(
            new vscode_1.LanguageModelTextPart('Error: No response message from v0 API'),
          )
        }
      } else {
  ...
  async promptForApiKey() {
    const result = await vscode_1.window.showInformationMessage(
      ...
      for (const toolCall of message.tool_calls) {
        if (toolCall.type === 'function' && toolCall.function) {
          try {
            ...
            progress.report(
              new vscode_1.LanguageModelToolCallPart(toolCall.id, toolCall.function.name, input),
            )
          } catch (error) {
            console.error('Failed to parse tool call arguments:', error)
            progress.report(
              new vscode_1.LanguageModelToolCallPart(toolCall.id, toolCall.function.name, {
