...
export function ProjectDropdown({
  ...
  const handleProjectSelect = (projectId: string) => {
    setOpen(false)
    if (projectId === 'new') router.push('/')
    else if (projectId !== currentProjectId) router.push(`/projects/${projectId}`)
  }
...
export function ChatDropdown({ projectId, currentChatId, chats, onChatChange }: ChatDropdownProps) {
  ...
  const handleChatSelect = async (chatId: string) => {
    ...
    if (chatId === 'new-from-scratch') {
      ...
      return
    } else if (chatId !== currentChatId) router.push(`/projects/${projectId}/chats/${chatId}`)
  ...
  const handleForkLatestChat = async () => {
    try {
      ...
      const response = await fetch('/api/chats/fork', {
        ...
        body: JSON.stringify({ chatId: latestChat.id, projectId }),
      })
      if (response.ok) {
