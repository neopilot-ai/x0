...
export function AgentBuilder() {
  ...
  const runTerminal = async () => {
    const response = await fetch('/api/terminal', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ command: terminalCommand, mode: terminalMode }) })
    const data = await response.json() as { output?: string; decision?: string }
    setTerminalOutput(`${data.decision ?? 'error'}\n\n${data.output ?? 'The terminal did not return output.'}`)
  }

  const refreshSandbox = async (action: 'status' | 'reset') => {
    const response = await fetch('/api/sandbox', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action }) })
    ...
    <main className="min-h-screen bg-background text-foreground">
      ...
      <div className="mx-auto grid max-w-7xl gap-6 p-4 md:grid-cols-[260px_minmax(0,1fr)_260px] md:p-8">
        <aside className="flex flex-col gap-4">
          ...
          <div className={panel}><div className={panelHeader}><p className="text-sm font-medium">Capabilities</p></div><div className={`${panelContent} flex flex-col gap-2 text-xs text-muted-foreground`}>{['Web search', 'Browser verification', 'Automatic error fixing', 'External integrations', 'Terminal commands'].map((item) => <div className="flex items-center justify-between rounded-md border border-border px-3 py-2" key={item}><span>{item}</span><Badge variant="outline">Ready</Badge></div>)}</div></div>
          <div className={panel}><div className={panelHeader}><p className="text-sm font-medium">Sandbox</p></div><div className={`${panelContent} text-xs text-muted-foreground`}><div className="flex items-center justify-between"><span className="capitalize">{sandboxStatus}</span><span className="size-2 rounded-full bg-emerald-500" /></div><p className="mt-2 leading-5">Isolated public demo workspace. No credentials, production access, or arbitrary writes.</p><Button className="mt-3 w-full" onClick={() => void refreshSandbox('reset')} variant="outline">Reset sandbox</Button></div></div>
        </aside>
        <section className="flex min-h-[70vh] flex-col rounded-2xl border border-border bg-card/30">
          ...
          <div className="border-t border-border bg-background/60 p-4"><div className="mb-3 flex items-center justify-between"><div className="flex gap-1"><Button type="button" size="sm" variant={activeTab === 'terminal' ? 'secondary' : 'ghost'} onClick={() => setActiveTab('terminal')}>Terminal</Button><Button type="button" size="sm" variant={activeTab === 'logs' ? 'secondary' : 'ghost'} onClick={() => setActiveTab('logs')}>Logs</Button></div><label className="flex items-center gap-2 text-xs text-muted-foreground">Permission<select className="rounded-md border border-border bg-background px-2 py-1" value={terminalMode} onChange={(event) => setTerminalMode(event.target.value as typeof terminalMode)}><option value="ask">Ask</option><option value="auto">Auto</option><option value="full">Full</option></select></label></div>{activeTab === 'terminal' ? <div><div className="flex gap-2"><input aria-label="Terminal command" className="min-w-0 flex-1 rounded-md border border-border bg-background px-3 py-2 font-mono text-xs" value={terminalCommand} onChange={(event) => setTerminalCommand(event.target.value)} /><Button type="button" onClick={() => void runTerminal()}>Run</Button></div><pre className="mt-3 max-h-28 overflow-auto rounded-md bg-foreground p-3 font-mono text-xs text-background">{terminalOutput}</pre></div> : <div className="rounded-md border border-dashed border-border p-3 text-xs text-muted-foreground">Agent events, approvals, tool calls, and sandbox transitions will appear here.</div>}</div>
